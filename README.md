...
attention and feedforward weights. Same input, same weights, different masks.
...
One weight set. Two execution modes. The encoder runs bidirectional — no mask, full context.
The decoder runs causally — causal mask, current position and left only.

There are no decoder-specific parameters. Every trainable parameter serves both modes simultaneously.
...
BPE tokenized, vocab_size=4096. Tokenizer trained on the corpus. The corpus is not included
in the repository. `datasets/sep.py` rebuilds it.

`gen0_language_structure.txt`: plain-language encodings of language regularities, used in
early training.
...
The encoder/decoder single-mode baselines use the gen0 dataset. The AIAYN baseline uses the
SEP corpus. All of this is configurable.

---

## Prior Work

Informed by encoder-decoder transformers and shared-parameter transformer variants.

The specific combination — bidirectional encoder and causal decoder sharing all backbone
weights with no decoder-specific parameters, differentiated purely by masking — does not
appear in the literature known to the author.

---

## License

Apache 2.0. The patent grant clause applies to this architecture. Attribution required on
derivatives.
